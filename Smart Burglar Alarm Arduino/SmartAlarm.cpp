...
String SmartAlarm::digitalClockDisplay() {
	...
	TimeStamp.concat(":" + String(minute()) + ":" + String(second()) + " " + String(dayShortStr(day())) + " " + String(monthShortStr(month())) + " " + String(year()));
	return TimeStamp;
}
void SmartAlarm::Timer()
{
	...
	input.start(2000); //Just testing if input will be detected in 3 seconds (2000 milliseconds should be 2 sec but here it adds 1 exra second)
	do {
		if (Check_Input_Timer.justFinished())
		{
